    ...
    stages {
        stage('Create  directory for the WEB Application')
        {
            steps{
                ...
                sh 'rm -rf "$WORKSPACE/app-web"'
                //Create the directory
                sh 'mkdir "$WORKSPACE/app-web"'
        ...
        stage('Create the Apache container') {
            ...
            sh 'docker run -dit --name app-web -p 9100:80  -v "$WORKSPACE/app-web":/usr/local/apache2/htdocs/ httpd'
            }
        }
        //Nginx webserver
        stage('Create the Nginx container') {
            ...
            steps {
            echo 'Creating the container...'
            sh 'docker run -dit --name app-web -p 9100:80  -v "$WORKSPACE/app-web":/usr/share/nginx/html nginx'
